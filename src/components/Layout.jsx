...
export default function Layout() {
  ...
  return (
    <div className={`min-h-screen ${theme ? 'dark-theme' : ''}`}>
      <header className="absolute top-0 w-full z-50 p-6 flex justify-between items-center max-w-7xl mx-auto left-0 right-0">
        <Link to="/" className="flex items-center gap-3 px-6 py-3 rounded-full hover:scale-105 transition-transform bg-black/30 backdrop-blur-md border border-white/20 shadow-lg">
          {/* SVG Hypercar Logo */}
          ...
          <span className="font-black text-2xl tracking-widest text-white uppercase drop-shadow-md">YM<span className="text-blue-500">.</span></span>
        </Link>
